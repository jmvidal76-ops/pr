define(['underscore', 'backbone', 'jquery', 'text!../../../html/envasado/StockMMPPSmile.html', 'compartido/notificaciones', 'compartido/utils',
        ...
        var gridStockMMPPSmile = Backbone.View.extend({
                    ...
                    self.dsStock = new kendo.data.DataSource({
                        transport: {
                            read: function (operation) {
                                ...
                                //Esto es para que no obtenga nada al iniciar.
                                //if (self.esInicio) {
                                //    operation.success([]);
                                //    resolve();
                                //    return;
            ...
            cargarGrid: function () {
                ...
                this.$("#gridStockMMPPSmile").kendoGrid({
                    ...
                    dataBound: function () { self.resizeGrid(); },
                    editable: true,
                    columns: [
                        {
                            template: function (e) {
            ...
            resizeGrid: function () {
                var alturaDisponible = $("#center-pane").height();
                ...
                // Altura máxima para el grid
                var alturaGrid = alturaDisponible - cabeceraHeight - filtrosHeaderHeight - footerHeight;
                if (alturaGrid < 150) alturaGrid = 150; // mínimo de seguridad
                $("#gridStockMMPPSmile").height(alturaGrid);

                // Ahora calcula el área scrolleable del grid
                var grid = $("#gridStockMMPPSmile").data("kendoGrid");
                if (grid && grid.wrapper) {
                    // Altura del paginador
                    var pagerHeight = grid.wrapper.find(".k-grid-pager").outerHeight(true);
                    if (!pagerHeight || isNaN(pagerHeight)) {
                        pagerHeight = 46; // Fallback si no hay paginador visible
                    }
                    var headerHeight = grid.thead ? grid.thead.outerHeight(true) : 0;
                    var toolbarHeight = grid.wrapper.find(".k-grid-toolbar").outerHeight(true) || 0;

                    // Área scrollable
                    var contentHeight = alturaGrid - pagerHeight - headerHeight - toolbarHeight;
                    if (contentHeight < 80) contentHeight = 80; // mínimo de seguridad

                    grid.wrapper.find(".k-grid-content").css({
