    ...
    function (_, Backbone, $, Keyboard, plantillaCambioPuesto, Not) {
        var VistaCambioPuesto = Backbone.View.extend({
            ...
            render: function () {
                ...
                //add for SEO
                this.$("#cmbPdvsSEO").kendoDropDownList({
                    dataTextField: "descript",
                    dataValueField: "ID",
                    dataSource: new kendo.data.DataSource({
                        data: window.app.SEO.pdvs,
                        sort: { field: "descript", dir: "asc" }
                    }),
                    optionLabel: window.app.idioma.t('SELECCIONE')
                });

                var ddlPDVSEOs = $("#cmbPdvsSEO").data("kendoDropDownList");
                ddlPDVSEOs.value(window.app.pdvSEOSel);
            ...
            aceptar: function () {
                ...
                //Add to ALT
                var indexPdvSelSEO = self.$("#cmbPdvsSEO option:selected").val();

                if (indexPdvSelSEO != -1) {                   
                    sesion.set("pdvSEO", indexPdvSelSEO);
                } else {
                    sesion.set("pdvSEO", null);
                }
