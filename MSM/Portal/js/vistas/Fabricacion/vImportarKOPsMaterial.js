define(['underscore', 'backbone', 'jquery', 'text!../../../fabricacion/html/ImportarKOPsPorMaterial.html', 'compartido/notificaciones', 'vistas/vDialogoConfirm', 'definiciones'],
    function (_, Backbone, $, plantilla, Notificacion, VistaDlgConfirm, definicion) {
        var vista = Backbone.View.extend({
            ...
            initialize: function (idZona, idTipoOrden) {
                ...
                self.render();
                $("#gridKOPs").data("kendoGrid").dataSource.data([]);
            },
            render: function () {
                var self = this;
                ...
                this.window = $(this.el).kendoWindow(
                    {
                        title: window.app.idioma.t('IMPORTAR_KOPS_POR_MATERIAL'),
                        width: "90%",
                        height: "90%",
                        modal: true,
                        resizable: false,
                ...
                $("#ddlMaterial").kendoDropDownList({
                    filter: "contains",
                    dataTextField: "DescripcionMosto",
                    dataValueField: "IdMaterial",
                    template: '#:data.IdMaterial #' + '#: data.DescripcionMosto != null ? " - " + data.DescripcionMosto : "" #',
                    ...
                    dataSource: {
                        transport: {
                            read: {
                                url: "../api/ObtenerMostosPorZonaTipo/" + self.idZona + "/" + self.idTipoOrden,
                                dataType: "json"
                            }
                        },
                        sort: { field: "NombreMosto", dir: "asc" },
                    },
                    select: function (e) {
                        var dataItem = this.dataItem(e.item);
                        var grid = $("#gridKOPs").data('kendoGrid');
                        self.idMaterialOrigenSeleccionado = dataItem.IdMaterial;
            ...
            cargaGridKOPs: function () {
                ...
                let gridKOPsDataSource = new kendo.data.DataSource({
                    type: "json",
                    transport: {
                        read: function (operation) {
                            var idMaterialOrigen = self.idMaterialOrigenSeleccionado; //$("#ddlMaterial").data('kendoDropDownList').value();
                            if (idMaterialOrigen === "") {
                                operation.success([]);
                            }
                            else {                                
                                $.ajax({
                                    url: "../api/ObtenerKOPSMostosPorZonaMostoTipoOrdenImportarPorMaterial/" + self.idZona + "/" + idMaterialOrigen + "/" + self.idTipoOrden,
                                    dataType: "json",
            ...
            ObtenerValor: function (datos, columna) {
                if (datos.Uom && datos.Uom.toUpperCase() == "TS") {
                ...
                } else if (datos.Tipo && datos.Tipo == "float") {
                    if (datos[columna] !== "") {
            ...
            resizeGrid: function (self, idGrid) {
                ...
                self.alturaGrid = self.alturaGrid == null ? ventanaEmergente - otherElementsHeight - divGrids - 185 : self.alturaGrid;
            ...
            Aceptar: function (e) {
                ...
                var idMaterialOrigen = $("#ddlMaterial").data('kendoDropDownList').value();
                var idMaterialEsIgual = dataItemMostosSeleccionado.indexOf(idMaterialOrigen) !== -1;

                if (dataItemMostosSeleccionado === "" || dataItemKOPsSeleccionado === "") {
                    Notificacion.crearNotificacion('error', window.app.idioma.t('INFORMACION'), window.app.idioma.t('SELECCIONAR_IMPORTAR_KOPS_MOSTO'), 5000);
                }
                else if (idMaterialOrigen !== '' && idMaterialEsIgual) {
                    kendo.ui.progress($("#divImportar"), false);
                    Notificacion.crearNotificacion('error', window.app.idioma.t('INFORMACION'), window.app.idioma.t('ERROR_IMPORTAR_KOPS_MATERIAL_ORIGEN_IGUAL_DESTINO'), 5000);
                }
                else {
                    self.confirmacion = new VistaDlgConfirm({
                        titulo: window.app.idioma.t('IMPORTAR_KOPS_POR'),
                        msg: window.app.idioma.t('CONFIRMAR_IMPORTAR_KOPS'),
                        funcion: function () {
                            self.ImportarKOPs(dataItemKOPsSeleccionado, dataItemMostosSeleccionado, idMaterialOrigen);
                        }, contexto: this
                    });
                }
            ...
            ImportarKOPs: function (KOPsSeleccionado, MostosSeleccionados, idMaterialOrigen) {
                var self = this;
                ...
                var Datos = {
                    ...
                    IdMaterialOrigen: idMaterialOrigen
                };
                ...
                $.ajax({
                    ...
                    type: "POST",
                    async: false,
                    url: "../api/ImportarKOPSPorMaterial",
