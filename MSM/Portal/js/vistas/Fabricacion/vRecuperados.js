define(['underscore', 'backbone', 'jquery', 'text!../../../Fabricacion/html/Recuperados.html', 'compartido/notificaciones'],
    function (_, Backbone, $, PlantillaRecuperados, Not) {
        var gridSilos = Backbone.View.extend({
            ...
            contador: 0,
            aPintar: 0,
            template: _.template(PlantillaRecuperados),
            initialize: function () {
                var self = this;

                $.ajax({
                    type: "GET",
                    url: "../api/GetResumenSilos/RECUPERADOS",
                    dataType: 'json',
                    cache: false,
                    async: true
                }).done(function (data) {
                ...
                //$.ajax({
                //    type: "GET",
                //    url: "../api/GetAreasGenericoSalas/RECU",
                //    dataType: 'json',
                //    cache: false,
                //    async: false
                //}).done(function (data) {
            ...
            render: function () {
                ...
                var tabStrip = this.$("#divPestaniasRecuperados").kendoTabStrip({
                    scrollable: true,
                    animation: {
                        open: {
                            effects: "fadeIn"
                ...
                for (i = 0; i < self.resumenSilos.length; i++) {
                    if (self.resumenSilos[i].ClassDescript.toString().toUpperCase().indexOf("RECUP") !== -1)
                        silosMalta.push(self.resumenSilos[i]);
                   
                }

                self.pintaSilos("recuperados", silosMalta);
            ...
            pintaSilos: function (padre, array) {
                ...
                switch (padre) {
                    case "recuperados":
                        ...
                        if (array.length > 0) {
                            self.$("#lblNombreMalta").html(window.app.idioma.t('TANQUES_TAPONADAS') + ":");
                ...
                if (array.length > 0) {
                    ...
                    var lblEstado = document.createElement("label");
                    $(lblEstado).text(window.app.idioma.t("TANQUES_VACIOS"));
