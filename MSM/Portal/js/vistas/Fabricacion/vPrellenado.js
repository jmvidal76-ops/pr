define(['underscore', 'backbone', 'jquery', 'text!../../../Fabricacion/html/Prellenado.html', 'compartido/notificaciones'],
    function (_, Backbone, $, PlantillaSilos, Not, vistaResumenSilos) {
        var gridSilos = Backbone.View.extend({
            ...
            initialize: function () {
                ...
                $.ajax({
                    type: "GET",
                    url: "../api/GetResumenSilos/PRELLENADO",
                    dataType: 'json',
                    cache: false,
                    async: true
                }).done(function (data) {
                ...
                //$.ajax({
                //    type: "GET",
                //    url: "../api/GetAreasGenericoSalas/PRE",
                //    dataType: 'json',
                //    cache: false,
                //    async: false
                //}).done(function (data) {
            ...
            render: function () {
                ...
                var tabStrip = this.$("#divPestaniasPrellenado").kendoTabStrip({
                    scrollable: true,
                    animation: {
                        open: {
                            effects: "fadeIn"
                ...
                for (i = 0; i < self.resumenSilos.length; i++) {
                    if (self.resumenSilos[i].Descript.toString().toUpperCase().indexOf("ENV.") !== -1)
                        silosMalta.push(self.resumenSilos[i]);
                   
                }

                self.pintaSilos("prellenado", silosMalta);
            ...
            pintaSilos: function (padre, array) {
                ...
                switch (padre) {
                    case "prellenado":
                        var divSilos = $("#maltas");
                        self.colores = ["#EBF5FB", "#D6EAF8", "#AED6F1", "#85C1E9", "#5DADE2", "#3498DB", "#2E86C1", "#2874A6", "#21618C"];
                        if (array.length > 0) {
                            self.$("#lblNombreMalta").html("TCP:");
                ...
                if (array.length > 0) {
                    ...
                    var lblEstado = document.createElement("label");
                    $(lblEstado).text(window.app.idioma.t("TCP_VACIOS"));
