define(['underscore', 'backbone', 'jquery', 'text!../../../Almacen/html/vpAsociarUbicacionZona.html', 'compartido/notificaciones', 'vistas/vDialogoConfirm', 'modelos/mSesion'],
    function (_, Backbone, $, plantilla, Not, VistaDlgConfirm, Session) {
        var vista = Backbone.View.extend({
            ...
            UbicacionesPorPlantilla: null,
            dsUbicaciones: null,
            window: null,
            dialog: null,
            //#endregion ATTRIBUTES

            initialize: function (plantilla,ubicaciones) {
                var self = this;
             

                self.PlantillaSeleccionada = plantilla;
                self.UbicacionesPorPlantilla = ubicaciones;

                self.dsUbicaciones = new kendo.data.DataSource({
                    async: false,
                    transport: {
                        read: {
                            url: "../api/ObtenerUbicacionesLogicas",
                            dataType: "json",
            ...
            render: function () {
                ...
                self.window = $(self.el).kendoWindow(
                    {
                        title: window.app.idioma.t('PLANTILLA_SELECCIONADA'),
                        width: "40%",
                        height: "80%",
                        modal: true,
                        resizable: false,
            ...
            reiniciarSeleccion: function () {
                ...
                grid.items().each(function (index, row) {
                    ...
                    if (data) {
                        ...
                        if (self.UbicacionesPorPlantilla.indexOf(data.IdUbicacion) !== -1) {
            ...
            guardarUbicacionesAsociadas: function (self) {
                ...
                const isEqual = (a, b) => JSON.stringify(ubicacionesSeleccionadas) === JSON.stringify(self.UbicacionesPorPlantilla);
                if (!isEqual(ubicacionesSeleccionadas, self.UbicacionesPorPlantilla)) {
            ...
            confirmarGuardado: function (self, ubicacionesSeleccionadas) {
                this.confirmacion = new VistaDlgConfirm({
                    ...
                    msg: window.app.idioma.t('DESEA_REALMENTE_ASIGNAR_UBICACIONES_PLANTILLAS'),
                    funcion: function () {
                        self.actualizarPlantillaUbicaciones(ubicacionesSeleccionadas);
                        Backbone.trigger('eventCierraDialogo');
                    },
                    contexto: this
                });
            },
            actualizarPlantillaUbicaciones: function (ubicacionesSeleccionadas) {
                var dataItem = {};
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    contentType: 'application/json; charset=utf-8',
                    url: '../api/ActualizarPlantillaUbicaciones',
                    success: function (result) {
                        if (result) {
                            self.window.close();
                            $("#contentUbicacionesPlantillas").data("kendoGrid").dataSource.read();
                            Not.crearNotificacion('success', window.app.idioma.t('AVISO'), window.app.idioma.t('UBICACIONES_ACTUALIZADAS_CORRECTAMENTE'), 4000);
                        }
                        else
                           Not.crearNotificacion('error', window.app.idioma.t('AVISO'), window.app.idioma.t('ERROR_ACTUALIZAR_UBICACIONESPLANTILLAS'), 4000);
                    },
                    error: function (e) {
                        Not.crearNotificacion('error', window.app.idioma.t('AVISO'), window.app.idioma.t('ERROR_ACTUALIZAR_UBICACIONESPLANTILLAS'), 4000);
                    }
