define(['underscore', 'backbone', 'jquery', 'text!../../../Envasado/html/TiemposCambio.html', 'vistas/vDialogoConfirm', 'compartido/notificaciones', 'jszip'],
    function (_, Backbone, $, PlantillaTiemposCambio, VistaDlgConfirm, Not, JSZip) {
        var tablaTiemposCambio = Backbone.View.extend({
            tagName: 'div',
            id: 'divHTMLContenido',
            template: _.template(PlantillaTiemposCambio),
            registrosSelData: [],
            registrosDesSelData: [],
            selTodos: false,
            initialize: function () {
            ...
            render: function () {
                ...
                self.dsTiempos = new kendo.data.DataSource({
                    ...
                    schema: {
                        model: {
                            id: "idTiempoCambio",
                            fields: {
                                idTiempoCambio: { type: "number" },
                                idLinea: { type: "number" },
                                descLinea: { type: "string" },
                                'productoEntrante.nombre': { type: "string" },
                                'productoEntrante.codigo': { type: "string" },
                                'productoSaliente.nombre': { type: "string" },
                                'productoSaliente.codigo': { type: "string" },
                                tiempo1: { type: "number" },
                                tiempo2: { type: "number" },
                                tiempoM: { type: "number" },
                                ...
                                'FormatoComunSaliente': { type: "string" },
                                'InhabilitarCalculo': { type: "boolean" }
                            }
                        }
                    },
                    sort: {
                        field: "idLinea", dir: "asc"
                    },
                    requestStart: function () {
                        if (self.dsTiempos.data().length == 0) {
                            kendo.ui.progress($("#gridTiemposCambio"), true);
                        }
                    },
                    requestEnd: function () {
                        if (self.dsTiempos.data().length == 0) {
                            kendo.ui.progress($("#gridTiemposCambio"), false);
                        }
                    },
                    error: function (e) {
                ...
                //Montamos Grid
                this.$("#gridTiemposCambio").kendoGrid({
                    dataSource: self.dsTiempos,
                    excel: {
                        fileName: window.app.idioma.t("TIEMPOS_DE_CAMBIO") + ".xlsx",
                        filterable: true,
                        allPages: true,
                    },
                    sortable: true,
                    ...
                    toolbar: [
                        ...
                        {
                            template: "<button type='button' id='btnCalcularTiempoCambioPreactor' style='float:right;' class='k-button k-button-icontext'><span class='k-icon k-i-custom'></span>" + window.app.idioma.t('CALCULA_TIEMPOS_SECUENCIADOR') + "</button>"
                        }
                    ],
                    columns: [
                        {
                        ...
                        {
                            field: "FormatoComunSaliente", title: window.app.idioma.t("FORMATO_COMUN_SALIENTE"), width: 160,
                            filterable: {
                                multi: true,
                                itemTemplate: function (e) {
                                    if (e.field == "all") {
                                    ...
                                    return "<div><label><input type='checkbox' value='#=FormatoComunSaliente#' style='width: 14px;height:14px;margin-right:5px;'/>#=FormatoComunSaliente#</label></div>";
                                }
                            }
                        },
                        {
                            field: "productoSaliente.codigo", title: window.app.idioma.t("CODIGO_PRODUCTO_SALIENTE"), width: 90,
                            filterable: {
                                ui: function (element) {
                        ...
                        {
                            field: "productoSaliente.nombre", title: window.app.idioma.t("PRODUCTO_SALIENTE"), width: 180
                        },
                        {
                        ...
                        {
                            field: "productoEntrante.codigo", title: window.app.idioma.t("CODIGO_PRODUCTO_ENTRANTE"), width: 90,
                            filterable: {
                                ui: function (element) {
                                    element.kendoNumericTextBox({
                                        format: "0",
                        ...
                        {
                            field: "productoEntrante.nombre", title: window.app.idioma.t("PRODUCTO_ENTRANTE"), width: 180
                        },
                        {
                            field: "tiempo1",
                    ...
                    dataBound: function (e, args) {
                        ...
                        var grid = $("#gridTiemposCambio").data("kendoGrid");
                ...
                var _grid = $("#gridTiemposCambio").data("kendoGrid");
                $("#gridTiemposCambio").data("kendoGrid").dataSource.bind("change", function (e) {
            ...
            aplicarSeleccion: function (checked) {
                ...
                if (self.selTodos) {
                    ...
                    var dataSource = $("#gridTiemposCambio").data("kendoGrid").dataSource;
            ...
            calcularTiempoCambioPreactor: function (e) {
                var self = this;
                ...
                if (self.selTodos) {
                    datos = $.map(dataFiltered, function (data, i) {
                        ...
                        datos.idProductoSaliente = data.productoSaliente.codigo;
                        datos.id = data.id;
                ...
                // Calcular Tiempo Cambio Preactor
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "../api/calcularTiempoCambioPreactor/",
                    dataType: 'json',
                    ...
                    if (res) {
                        Not.crearNotificacion('success', window.app.idioma.t('INFO'), window.app.idioma.t('SE_HAN_MODIFICADO_TIEMPO_CAMBIO_SECUENCIADOR'), 4000);
                    } else {
                        Not.crearNotificacion('error', window.app.idioma.t('ERROR'), window.app.idioma.t('ERROR_AL_MODIFICAR_TIEMPO'), 4000);
                    }
                ...
                // Calcular Tiempo Cambio Medio
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "../api/calcularTiempoCambioMedio/",
                    dataType: 'json',
                    data: JSON.stringify(datos),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json"
                    ...
                    if (res) {
                        Not.crearNotificacion('success', window.app.idioma.t('INFO'), window.app.idioma.t('SE_HAN_MODIFICADO_TIEMPO_CAMBIO_MEDIO'), 4000);
                    } else {
                        Not.crearNotificacion('error', window.app.idioma.t('ERROR'), window.app.idioma.t('ERROR_AL_MODIFICAR_TIEMPO_CAMBIO_MEDIO'), 4000);
                    }
                    Backbone.trigger('eventCierraDialogo');
                }).error(function (err) {
                    Backbone.trigger('eventCierraDialogo');
            ...
            confirmAsignarParametros: function (e) {
                ...
                if (grid.tbody.find('>tr.k-state-selected').length > 0 || self.registrosSelData.length > 0 ||
                    (self.registrosDesSelData.length > 0 && self.registrosDesSelData.length < self.dsTiempos.data().length)) {
                        if (e.currentTarget.id == "btnCalcularTiempoCambioPreactor") {
                            this.confirmacion = new VistaDlgConfirm({
                                titulo: window.app.idioma.t('TIEMPOS_DE_CAMBIO'),
                                msg: window.app.idioma.t('DESEA_REALMENTE_RECALCULAR_EL_TIEMPO'),
                                funcion: function () {
                                    self.calcularTiempoCambioPreactor(e.currentTarget.id);
                                },
                                contexto: this
                            });
                        }
            ...
            confirmAsignarCambios: function (e) {
                var self = this;
                var permiso = TienePermiso(3);
                ...
                if (grid.tbody.find('>tr.k-state-selected').length > 0 || self.registrosSelData.length > 0 ||
                    (self.registrosDesSelData.length > 0 && self.registrosDesSelData.length < self.dsTiempos.data().length)) {
                        this.confirmacion = new VistaDlgConfirm({
                            titulo: window.app.idioma.t('TIEMPOS_DE_CAMBIO'),
                            msg: window.app.idioma.t('DESEA_REALMENTE_CAMBIAR_LOS_TIEMPOS_DE'),
                            funcion: function () {
                                self.AsignarCambios(e.currentTarget.id);
                            },
                            contexto: this
                        });
            ...
            AsignarCambios: function () {
                        ...
                        if (!_.findWhere(self.registrosDesSelData, datos)) {
                            return {
                                ...
                                lineaProducto: window.app.idioma.t('LINEA') + ' ' + datos.numeroLinea + ' - ' +
                                    ...
                                    window.app.idioma.t('PRODUCTO_SALIENTE') + ': ' + datos.idProductoSaliente
                            };
                        }
                    });
                } else {
                ...
                $.ajax({
                    ...
                    if (res) {
                        self.dsTiempos.read();
                        Not.crearNotificacion('success', window.app.idioma.t('INFO'), window.app.idioma.t('SE_HAN_ASIGNADO_TIEMPO_CAMBIO'), 4000);
                    } else {
                        Not.crearNotificacion('error', window.app.idioma.t('ERROR'), window.app.idioma.t('ERROR_ASIGNANDO_LOS_TIEMPOS'), 4000);
                    }
                    Backbone.trigger('eventCierraDialogo');
                }).error(function (err) {
                    Backbone.trigger('eventCierraDialogo');
            ...
            resizeGrid: function () {
                ...
                var gridElement = $("#gridTiemposCambio"),
