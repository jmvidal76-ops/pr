define(['underscore', 'backbone', 'jquery', 'text!../../../Envasado/html/EnvioEmailPartesCalidadPendientes.html', 'compartido/notificaciones', 'vistas/vDialogoConfirm'],
    function (_, Backbone, $, plantilla, Not, VistaDlgConfirm) {
        var vistaEmailPartesPendientes = Backbone.View.extend({
            template: _.template(plantilla),
            dsPartesPendientes: null,
            listaPdvs: [],
            initialize: function () {
                var self = this;

                self.dsPartesPendientes = new kendo.data.DataSource({
                    pageSize: 50,
                    transport: {
                        read: {
                            url: "../api/MailPartesCalidadPendientes_Read",
                            dataType: "json"
                        },
                        create: {
                            url: "../api/MailPartesCalidadPendientes_Create",
                            ...
                            complete: function (e) {
                                if (e.status == 500) {
                                    ...
                                    $("#gridPartesPendientes").data("kendoGrid").dataSource.read();
                                }
                            },
                        },
                        update: {
                            url: "../api/MailPartesCalidadPendientes_Update",
                            contentType: 'application/json;charset=utf-8',
                            dataType: "json",
                            type: "PUT",
                            complete: function (e) {
                        ...
                        destroy: {
                            url: "../api/MailPartesCalidadPendientes_Delete",
                            contentType: 'application/json;charset=utf-8',
                            dataType: "json",
                            type: "DELETE",
                            complete: function (e) {
            ...
            render: function () {
                ...
                self.grid = this.$("#gridPartesPendientes").kendoGrid({
                    dataSource: self.dsPartesPendientes,
                    sortable: true,
                    resizable: true,
                    scrollable: true,
                    pageable: {
                            ...
                            command: [
                                {
                                    ...
                                    click: function (e) {
                                        var permiso = TienePermiso(378);

                                        if (!permiso) {
                                            Not.crearNotificacion('warning', window.app.idioma.t('AVISO'), window.app.idioma.t('AVISO_SIN_PERMISOS'), 3000);
                                            $('#gridPartesPendientes').data("kendoGrid").cancelChanges();
                                        }
                                    }
                                },
                                {
                                    ...
                                    click: function (e) {
                                        ...
                                        var grid = $("#gridPartesPendientes").data("kendoGrid");
                                ...
                                },
                                {
                                    ...
                                    click: function (e) {
                                        kendo.ui.progress($("#gridPartesPendientes"), true);
                                        e.preventDefault(); //prevent page scroll reset
                                        var tr = $(e.target).closest("tr");
                                        var data = this.dataItem(tr);
                                        self.testMail(data);
            ...
            testMail: function (data) {
                $.ajax({
                    type: "GET",
                    url: "../api/MailPartesCalidadPendientes_TestMail/" + data.IdMailParteCalidad,
                    dataType: 'json',
                    cache: false,
                    async: false
                }).done(function (data) {
                    Not.crearNotificacion('success', 'Info', window.app.idioma.t('SUCCESS_TEST_MAIL'), 4000);
                    kendo.ui.progress($("#gridPartesPendientes"), false);
                }).fail(function (xhr) {
            ...
            resizeGrid: function () {
                ...
                var gridElement = $("#gridPartesPendientes"),
        ...
        return vistaEmailPartesPendientes;
    });