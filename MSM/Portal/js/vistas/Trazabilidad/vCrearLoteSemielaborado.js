define(['underscore', 'backbone', 'jquery', 'text!../../../Trazabilidad/html/CrearLoteSemielaborado.html', 'compartido/notificaciones', 'vistas/vDialogoConfirm', '../../../../Portal/js/constantes'],
    function (_, Backbone, $, plantillaCrearEditarUbicacion, Not, VistaDlgConfirm, enums) {
        var vistaCrearLoteSemielaborado = Backbone.View.extend({
            ...
            renderElementsFilters: function () {
                ...
                $("#cmbTipoLote").kendoDropDownList({
                    ...
                    select: function (e) {
                        ...
                        if (dataItem.IdProcesoLote === self.constProcesoLoteFullNames.Coccion) {
                            cmbUbicacion.dataSource.transport.options.read.url = "../api/ObtenerUbicaciones/" + self.defaultIdTipoUbicacionCoccion + "/" + idTipoZona;
                        ...
                        if (dataItem.IdProcesoLote === self.constProcesoLoteFullNames.Filtracion) {
                            const promises = [];

                            promises.push(self.obtenerMatSemielaboradosClaseCervezaFiltrada(self.defaultIdTipoMaterial, dataItem.IdClaseMaterial));
                            promises.push(self.obtenerMatSemielaboradosClaseMostos(self.defaultIdTipoMaterial));

                            Promise.all(promises).then(values => {
                                let dataMateriales = values[0].concat(values[1]);
                                comboReferencia.dataSource.data(dataMateriales);
                                comboReferencia.select(0);
                            }).catch(err => {
                                Not.crearNotificacion('error', window.app.idioma.t('ERROR'), window.app.idioma.t('ERROR_MATERIALES'), 3000);
                            });
                        } else {
                    ...
                    dataBound: function (e) {
                        ...
                        if (idTipoLote != '') {
                            ...
                            if (zonasGrouped && Object.keys(zonasGrouped).length === 1)
                                self.idZona = Object.keys(zonasGrouped)[0];

                            if (almacenesGrouped && Object.keys(almacenesGrouped).length === 1)
                                idAlmacen = Object.keys(almacenesGrouped)[0];

                            if (self.idZona == 0) {
                                $("#cmbZona").data("kendoDropDownList").dataSource.transport.options.read.url = "../api/GetZone/" + idAlmacen;
                            } else {
                                $("#cmbZona").data("kendoDropDownList").dataSource.transport.options.read.url = "../api/GetZone/0"
                            }

                            $("#cmbZona").data("kendoDropDownList").dataSource.read();

                            if (dataUbicaciones.length === 1) {
                                $("#cmbUbicacion").data("kendoDropDownList").value(dataUbicaciones[0].IdUbicacion);
                                self.idZona = dataUbicaciones[0].Zona[0].IdZona;
                ...
                cmbClaseMat.value("MOS"); // Por defecto se preestablece Mosto
            ...
            obtenerMatSemielaboradosClaseCervezaFiltrada: async function (idTipoMaterial, idClaseMaterial) {
                return new Promise((resolve) => {
                    $.ajax({
                        type: "GET",
                        url: "../api/GetMaterial/" + idTipoMaterial + "/" + idClaseMaterial,
                        dataType: 'json',
            ...
            obtenerMatSemielaboradosClaseMostos: async function (idTipoMaterial) {
                return new Promise((resolve) => {
                    $.ajax({
                        type: "GET",
                        url: "../api/GetMaterial/" + idTipoMaterial + "/" + "MOS",
                        dataType: 'json',
                        success: function (data) {
                            resolve(data);
                        },
